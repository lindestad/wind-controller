...
Reproducible ESP32-C3 / Zephyr firmware and KiCad PCB for a four-fan simracing wind simulator.

![Populated WindPCB B.3 CAD render](hardware/reports/board-final.png)

[Hardware and manufacturing files](hardware/README.md) · [Schematic](hardware/reports/schematic.pdf) · [BOM](hardware/BOM.csv) · [Host integration](docs/host-integration.md)

The images are CAD renderings; representative USB/jack/eFuse models are visual aids.
...
The assembled B.3 board is working. On 22 September 2026 it was programmed over native USB, tested with one and then two fans, and integrated with SimHub through Rig Companion. Live full-demand readings were approximately 2,470 and 2,410 RPM (fan rating 2,500 RPM); the user confirmed good airflow after correcting a reversed fan. This is functional bring-up evidence, not calibrated electrical or thermal qualification. See [validation scope](docs/validation.md).

## Repository layout

- `src/`, `include/`, `boards/`: firmware and custom Zephyr board definition.
- `hardware/`: native KiCad project, local libraries/models, BOM, Gerbers, CPL, STEP, PDFs, renders and exporter.
- `scripts/`, `tests/`: dependency setup, build, pin audit, stop-first flash and executable host tests.
- `integrations/simhub/`: source snapshot of the companion's SimHub bridge; no proprietary SDK DLLs.
- `reports/`: historical software/build/bring-up evidence, not a substitute for rerunning checks.
...
Install Git, Python 3.12 (including `py`), CMake, Ninja and a host GCC (for the tests; Strawberry Perl's GCC works on Windows). Install the complete **Zephyr SDK 1.0.1** at `~/zephyr-sdk-1.0.1`, or pass its directory with `-Sdk`. Fetch the pinned source dependencies once:
